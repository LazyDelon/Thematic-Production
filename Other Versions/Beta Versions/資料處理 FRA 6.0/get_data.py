...
def getstock():
    ...
    with open('Stock_Number_Code.csv', encoding="utf-8-sig") as F:
        Flist = F.readlines()
        # for lst in Flist: # 走訪每一個股票組合
        #     F = lst.split(',')
            
        #     res_F.append([F[0].strip(),str(F[1].strip())]) 

    with open('Stock_Number_Listed.csv', encoding="utf-8-sig") as L:
        Llist = L.readlines()
        # for lst in Llist: # 走訪每一個股票組合
        #     L = lst.split(',')
            
        #     res_L.append([L[0].strip(),str(L[1].strip())]) 
    ...
    result1 = Counter([key for key,value in result.items()if value > 1])
...
def get_data(s, path, number, group):
    # try:
    File_Path = (path+"\\html\\tifrs-"+s+"\\"+number+".html")
...
'''def get_roa_data(s, path, number, group):
    ...
    if result == True:
        ...
        try:
            for index_num in range(20,50): #取得陣列中的資料

                file_num = str(index_num + 1)
                
                # 查詢是否已有資料夾，無資料夾則新增一個
                folder = path + group + "\\" + s + "\\" + number + "\\ROA"
                if not os.path.isdir(folder):
                    os.makedirs(folder)
...
def combine_all_data(s, path, number, name, group):
    ...
    if result == True:
        ...
        datalist = ["2", "權益總計 Total stockholders' equity", "權益總計 Equity", "權益總計 Total equity", "權益總額 Total equity"]
        ...
        df = pd.concat(
                (pd.read_csv(file,header=0, usecols=["權益總計 Total stockholders' equity", "權益總計 Equity", "權益總計 Total equity", "權益總額 Total equity"]
        ...
        df.columns = ['上季權益總計', '上季權益總計', '上季權益總計', '上季權益總計']
        ...
        datalist = ['2', '本期淨利（淨損）Profit (loss)']
        ...
        df = pd.concat(
                (pd.read_csv(file,header=0, usecols=['本期淨利（淨損）Profit (loss)']
                                ,dtype={'name': str, 'tweet': str}) for file in files), sort=False, ignore_index=True).replace( '[(]','-',  regex=True).replace( '[,]','',  regex=True).replace( '[)]','',  regex=True)
        ...
        df.columns = ['2', '資產總計　Total assets', "權益總計 Total stockholders' equity", "權益總計 Equity", "權益總計 Total equity", "權益總額 Total equity"]
        ...
        datalist = ['2','繼續營業單位稅前淨利（淨損）Profit (loss) from continuing operations before tax','本期淨利（淨損）Profit (loss)']
...
'''def combine_roa_data(s, path, number, name, group):
    ...
    try: 
        for x in range(21,50):
            ...
            df = pd.read_csv(folder+"\\"+number+"_"+file_num+".csv", encoding= "utf_8_sig") #取得csv檔案位置
            ...
            #刪除num_list陣列的3、4
            del num_list[1:2] #刪除num_list陣列的3
            del num_list[1:2] #刪除num_list陣列的4
            ...
            data = pd.DataFrame(data, index=index1)
            data.to_csv(folder + "\\"+number+"_getfile_"+file_num+".csv",header=0, encoding= "utf_8_sig") #存檔
    except:
        pass

    files = glob(folder+"\\"+number+"_getfile_*.csv") #2
    df = pd.concat((pd.read_csv(file, dtype={'name': str, 'tweet':str}) for file in files), ignore_index=True)
    df.to_csv(folder+"\\"+number+"_merge.csv", index = False, encoding='utf_8-sig')

    df = pd.read_csv(folder+"\\"+number+"_merge.csv",usecols=["2","利息收入合計"]) #3
    df=df.dropna()
    df.to_csv(path + group + "\\" + s + "\\" + number+"\\"+number+"_getfile_4.csv", index = False, encoding='utf_8-sig')

    files = glob(path + group + "\\" + s + "\\" + number+"\\"+number+"_getfile_*.csv") #取得資料夾內需要的資料  #4
    ...
    result = pd.merge(result,df_list[3], on='2')
    ...
    result.to_csv(path + group + "\\" + s + "\\" + number + "\\整合\\"+"stock_"+number+".csv", encoding='utf_8-sig', index = False) #存檔
'''
...
'''
        ...
        df.columns = ['資產總計','權益總計','上季權益總計', '稅前淨利','本期淨利','累季本期淨利'
                    , '本期稅前淨利','利息費用','營業活動之淨現金流入','股票代碼','股票名稱','類股','季別','判斷季別']
...
def calculate(s, num, season, path, number, group):
    ...
    for i in range(0,len(os.listdir(Path))):
        ...
        if Folder == '2019Q1' or Folder == '2020Q1' or Folder == '2021Q1':
            ...
            Income_Net1 = Data_File.稅前淨利.astype(int64)
        ...
        if Folder == '2019Q2' or Folder == '2020Q2' or Folder == '2021Q2':
            ...
            Interest_Expense1 = Data_File1.利息費用.astype(int64)

            Income_Net2 = Data_File.稅前淨利.astype(int64)
            Interest_Expense2 = Data_File.利息費用.astype(int64)

            
            Interest_Result = (Interest_Expense2 - Interest_Expense1)

            Income_Result = (Income_Net2 + Interest_Result) / Interest_Result
        ...
        if Folder == '2019Q3' or Folder == '2020Q3' or Folder == '2021Q3':
            ...
            Interest_Expense2 = Data_File1.利息費用.astype(int64)

            Income_Net3 = Data_File.稅前淨利.astype(int64)
            Interest_Expense3 = Data_File.利息費用.astype(int64)

            Interest_Result = (Interest_Expense3 - Interest_Expense2)

            Income_Result = (Income_Net3 + Interest_Result) / Interest_Result
        ...
        if Folder == '2019Q4' or Folder == '2020Q4' or Folder == '2021Q4':
            ...
            Income_Net1 = Data_File1.稅前淨利.astype(int64)
            Interest_Expense1 = Data_File1.利息費用.astype(int64)
            ...
            Income_Net2 = Data_File2.稅前淨利.astype(int64)
            ...
            Income_Net3 = Data_File3.稅前淨利.astype(int64)


            Income_Net4 = Data_File.稅前淨利.astype(int64)
            Interest_Expense4 = Data_File.利息費用.astype(int64)

            Income_Result4 = (Income_Net4 - Income_Net1 - Income_Net2 - Income_Net3)
            Interest_Result4 = (Interest_Expense4 - Interest_Expense1)
