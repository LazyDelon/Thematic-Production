...
def get_data(path,number,group):
    df = pd.read_html(path+"\\html\\tifrs-2020Q1\\"+number+".html")#取得放置html的資料夾
    # 查詢是否已有資料夾，無資料夾則新增一個
    ...
    df[2].to_csv(path+group+"\\"+number+"//"+number+"_3.csv",header=0, encoding= "utf_8_sig")
...
def combine_all_data(path,number,name,group):
    ...
    for i in range(1,4):
        #取得csv檔案位置
        a = str(i)
        df = pd.read_csv(path+group+"\\"+number+"//"+number+"_"+a+".csv", encoding= "utf_8_sig")
        #取得csv表格列數
        ...
        #存檔
        data.to_csv(path+group+"\\"+number+"//"+number+"_new_"+a+".csv",header=0, encoding= "utf_8_sig")
    
    
    files = glob(path+group+"\\"+number+"//"+number+"_new_1.csv")
    df = pd.concat(
            (pd.read_csv(file,header=0, usecols=['3','應收帳款淨額 Accounts receivable, net','流動資產合計 Total current assets','流動負債合計 Total current liabilities',
                                                 '存貨 Current inventories','不動產、廠房及設備 Property, plant and equipment',
                                                 '資產總計　Total assets','負債總計 Total liabilities']
                            ,dtype={'name': str, 'tweet':str}) for file in files), sort=False, ignore_index=True)#應收帳款
    #存檔
    df.to_csv(path+group+"\\"+number+"//"+number+"_getnum_1.csv", encoding= "utf_8_sig", index = False)
    

    files = glob(path+group+"\\"+number+"//"+number+"_new_2.csv")
    df = pd.concat(
    ...
    #存檔
    df.to_csv(path+group+"\\"+number+"//"+number+"_getnum_2.csv", encoding= "utf-8-sig", index = False)
    

    files = glob(path+group+"\\"+number+"//"+number+"_new_3.csv")
    df = pd.concat(
    ...
    #存檔
    df.to_csv(path+group+"\\"+number+"//"+number+"_getnum_3.csv", encoding= "utf-8-sig", index = False)
    
    #取得資料夾內所有資料
    files = glob(path+group+"\\"+number+"//"+number+"_getnum_*.csv")
    ...
    result["股票名稱"] = name   
    # 查詢是否已有資料夾，無資料夾則新增一個
    folder = ""+path+group+"\\計算"
    if not os.path.isdir(folder):
        os.makedirs(folder)
    #存檔
    
    result.to_csv(path+group+"\\計算\\stock_"+number+".csv", encoding='utf-8-sig', index = False)


def csv_db(group_name,path):

    files = glob(path+group_name+"\\計算\\stock_*.csv")
    ...
    df = pd.concat(
            (pd.read_csv(file,header=0, usecols=['應收帳款淨額 Accounts receivable, net','流動資產合計 Total current assets','流動負債合計 Total current liabilities',
                                                 ...
                                                 '營業收入合計　Total operating revenue','利息費用 Interest expense','股票代碼', '股票名稱']
                            ,dtype={'name': str, 'tweet': str}) for file in files), sort=False, ignore_index=True).replace( '[(]','-',  regex=True).replace( '[,]','',  regex=True).replace( '[)]','',  regex=True)

    df.columns = ['應收帳款淨額','存貨','流動資產合計','不動產、廠房及設備','資產總計','流動負債合計','負債總計','營業收入合計',
                  '本期淨利（淨損）','本期稅前淨利（淨損）','利息費用','營業活動之淨現金流入（流出）','股票代碼','股票名稱']

    df.to_csv(path+"Sort_All\\"+group_name+".csv", encoding='utf-8-sig', index = False)
    
    conn = sqlite3.connect(path + "\\DataBase"+group_name+"財務分析資料.db")
    df = pd.read_csv(path+"Sort_All\\"+group_name+".csv", encoding="utf-8-sig") #, dtype='float64'

    df.to_sql("sort_2020Q1", conn, if_exists="replace", index=False,
    dtype={
        "本期淨利（淨損）": "float64",
        "本期稅前淨利（淨損）": "float64",
