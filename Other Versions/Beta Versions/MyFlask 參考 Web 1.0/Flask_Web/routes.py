...
def shipping_business():
    if request.method == 'GET':
        conn = sqlite3.connect("2019Q2/半導體業.db")
        cursor = conn.cursor()
        sql="SELECT 股票代碼, 股票名稱,ROUND((本期淨利 / ((資產總計 - 負債總計)/2)),2) as ROE, \
            ROUND((流動資產合計 / 流動負債合計)*100,2) as 流動比率, \
            ROUND(((流動資產合計 - 存貨) / 流動負債合計)*100,2) as 速動比率, \
            ROUND(((本期稅前淨利 + 利息費用) / 利息費用),2) as 利息保障倍數, \
            ROUND((營業活動之淨現金流入 / 流動負債合計)*100,2) as 現金流量比率, \
            ROUND((營業收入合計 / ((應收帳款淨額 + 應收帳款淨額)/2)),2) as 應收款項週轉率, \
            ROUND((營業成本合計 / ((存貨 + 存貨)/2)),2) as 存貨週轉率, \
            ROUND((營業收入合計 / ((不動產、廠房及設備 + 不動產、廠房及設備)/2)),2) as 固定資產週轉率, \
            ROUND((營業收入合計 / 資產總計),2) as 總資產週轉率, \
            ROUND((負債總計 / 資產總計),2) as 負債佔資產比率, \
            ROUND((本期淨利 / 營業收入合計),2) as 純益率 FROM 半導體業"
        cursor.execute(sql)
        content = cursor.fetchall()
