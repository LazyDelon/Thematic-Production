...
def calculate():
    if request.method == "GET":
        ...
        if ID == None:
            sql = "SELECT 季, 股票類別, 股票代碼, 股票名稱, ROE FROM Stock WHERE 季 = '2021Q3' ORDER BY ROE DESC"
            cur.execute(sql)
            content = cur.fetchall()
            field_name = [i[0] for i in cur.description]

        else:
            sql = "SELECT 季, 股票類別, 股票代碼, 股票名稱, ROE FROM Stock WHERE 季 = '2021Q3' AND 股票類別='{}' ORDER BY ROE DESC".format(ID)
            cur.execute(sql)
            content = cur.fetchall()
            field_name = [i[0] for i in cur.description]
