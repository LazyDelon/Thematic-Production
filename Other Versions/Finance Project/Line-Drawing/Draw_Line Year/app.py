...
def search():
    ...
    sql = "SELECT 年度,年度ROE,年度ROA FROM Stock WHERE 股票代碼 = " + str(key)
    cursor = conn.cursor()
    cursor.execute(sql)
    content_year = cursor.fetchall()
    field_name_year = [i[0] for i in cursor.description]
    cursor.execute(sql)
    sql = cursor.fetchall()
    none_data = []
    roa_res_year = []
    ...
    return render_template('search.html', labels=field_name,labels_four = field_name_four,labels_year = field_name_year,
                                            content=content,
                                            stock = key,
                                            data_plot = data_plot,
                                            data_plot_d = data_plot_d,content_four = content_four,
                                            data_plot_y = data_plot_y,content_year = content_year)
