...
def ROA_read_folder_saveCSV(path,html_path,number,group):
    for i in range(0,len(os.listdir(html_path))):
        folder = (os.listdir(html_path)[i]) #取得陣列中的資料夾名稱
        
        df = pd.read_html(html_path+"\\"+folder+"\\"+number+".html")

        try:
            for index_num in range(20,50):
                file_num = str(index_num+1)

                Path_CSV = path+"\\"+group+"\\"+folder+"\\"+number+"\\ROA" #設定資料存放路徑
                if not os.path.isdir(Path_CSV): #判斷資料夾是否存在
                    os.makedirs(Path_CSV) #新增資料夾
...
def ROA_combine_all_data(path,html_path,number,name,group):
    for i in range(0,len(os.listdir(html_path))):
        folder = (os.listdir(html_path)[i]) #取得陣列中的資料夾名稱

        Paths = path+"\\"+group+"\\"+folder+"\\"+number #設定資料存放路徑
        if not os.path.isdir(Paths): #判斷資料夾是否存在
            os.makedirs(Paths) #新增資料夾

        Path_CSV = Paths+"\\ROA" #設定資料存放路徑
        if not os.path.isdir(Path_CSV): #判斷資料夾是否存在
            os.makedirs(Path_CSV) #新增資料夾

        folder_save = Paths+"\\計算"
        if not os.path.isdir(folder_save): #判斷資料夾是否存在
            os.makedirs(folder_save) #新增資料夾
        ...
        files = glob(Path_CSV+"\\"+number+"_getfile_*.csv") #2
        df = pd.concat((pd.read_csv(file, dtype={'name': str, 'tweet':str}) for file in files), ignore_index=True)
        df.to_csv(Path_CSV+"\\"+number+"_merge.csv", index = False, encoding='utf_8-sig')
    
        df = pd.read_csv(Path_CSV+"\\"+number+"_merge.csv",usecols=["2","利息收入合計"]) #3
        df=df.dropna()
        df.to_csv(Paths+"\\"+number+"_getfile_4.csv", index = False, encoding='utf_8-sig')

        files = glob(Paths+"\\"+number+"_getfile_*.csv") #取得資料夾內需要的資料  #4
        df_list = [pd.read_csv(file) for file in files] #串列中包含兩個Pandas DataFrame
        result = pd.merge(df_list[0],df_list[1], on='2') #進行資料合併
        result = pd.merge(result,df_list[2], on='2') #進行資料合併
...
def calculate_ROA(path,number,html_path,group):
    Path = path+"\\"+group
    for i in range(0,len(os.listdir(html_path))):
        folder = (os.listdir(html_path)[i])
        Path_CSV = Path+"\\"+folder+"\\"+number
        ...
        if folder == '2019Q1':
            ...
            a = (df.本期淨利.astype(int64))
            b = (df.利息收入合計.astype(int64))
            c = (df.資產總計.astype(int64))
    ...
    for i in range(0,len(os.listdir(html_path))):
        ...
        df = pd.concat(
                (pd.read_csv(file,header=0, usecols=['應收帳款淨額',
                    ...
                    '利息費用',
                    '營業活動之淨現金流入',
                    '股票代碼','股票名稱','股票類別','季','流動比率','速動比率','利息保障倍數','現金流量比率','總資產週轉率','負債佔資產比率','純益率','ROE','ROA']
                                ,dtype={'name': str, 'tweet':str}) for file in files),
                sort=False, ignore_index=True).replace( '[(]','-',  regex=True).replace( '[,]','',  regex=True).replace( '[)]','',  regex=True)
        df.columns = ['應收帳款淨額',
                    '存貨',
...
def calculate_ROE_4(path,number,html_path,group):
    Path = path+"\\"+group
    for i in range(0,len(os.listdir(html_path))):
        folder = (os.listdir(html_path)[i])
        Path_CSV = Path+"\\"+folder+"\\"+number
        ...
        if folder == '2019Q1' or folder == '2019Q2' or folder == '2019Q3':
            a = 0

            df['ROE_近四季'] = a

        elif folder == '2019Q4' or folder == '2020Q1' or folder == '2020Q2' or folder == '2020Q3' or folder == '2020Q4' or folder == '2021Q1' or folder == '2021Q2' or folder == '2021Q3':
            folder1 = (os.listdir(html_path)[i-1])
            Path_CSV1 = Path+"\\"+folder1+"\\"+number
            df1 = pd.read_csv(Path_CSV1+"\\計算\\stock_"+number+".csv")
            a1 = (df1.ROE.astype(int64))

            folder2 = (os.listdir(html_path)[i-2])
            Path_CSV2 = Path+"\\"+folder2+"\\"+number
            df2 = pd.read_csv(Path_CSV2+"\\計算\\stock_"+number+".csv")
            a2 = (df2.ROE.astype(int64))

            folder3 = (os.listdir(html_path)[i-3])
            Path_CSV3 = Path+"\\"+folder3+"\\"+number
            df3 = pd.read_csv(Path_CSV3+"\\計算\\stock_"+number+".csv")
            a3 = (df3.ROE.astype(int64))

            a = (df.ROE.astype(int64))
            a = (a+a1+a2+a3)
...
def calculate_ROA_4(path,number,html_path,group):
    Path = path+"\\"+group
    for i in range(0,len(os.listdir(html_path))):
        folder = (os.listdir(html_path)[i])
        Path_CSV = Path+"\\"+folder+"\\"+number
        ...
        if folder == '2019Q1' or folder == '2019Q2' or folder == '2019Q3':
            ...
            Path_CSV1 = Path+"\\"+folder1+"\\"+number
            df1 = pd.read_csv(Path_CSV1+"\\計算\\stock_"+number+".csv")
            a1 = (df1.ROA.astype(int64))

            folder2 = (os.listdir(html_path)[i-2])
            Path_CSV2 = Path+"\\"+folder2+"\\"+number
            df2 = pd.read_csv(Path_CSV2+"\\計算\\stock_"+number+".csv")
            a2 = (df2.ROA.astype(int64))

            folder3 = (os.listdir(html_path)[i-3])
            Path_CSV3 = Path+"\\"+folder3+"\\"+number
            df3 = pd.read_csv(Path_CSV3+"\\計算\\stock_"+number+".csv")
            a3 = (df3.ROA.astype(int64))

            a = (df.ROA.astype(int64))
            a = (a+a1+a2+a3)
...
def csv_db(path,html_path,number,group):
    ...
    for i in range(0,len(os.listdir(html_path))):
        ...
        conn = sqlite3.connect(Sql_data+"\\Stock.db")
