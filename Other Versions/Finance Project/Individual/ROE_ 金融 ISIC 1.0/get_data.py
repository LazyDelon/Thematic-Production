...
def csv_data(path,number,html_path,group):
    ...
    for i in range(0,len(os.listdir(html_path))):
        ...
        if result == True:
            ...
            pass
def calculate(path,number,html_path,group): #ROE
    Path = path+"\\"+group
    for i in range(0,len(os.listdir(html_path))):
        ...
        if result == True:
            ...
            if i == 0 or i == 1 or i == 2 :
                roe_four = None
            elif folder == '2019Q4' or folder == '2020Q4' or folder == '2021Q4':
                folder1 = (os.listdir(html_path)[i-3])
                Path_CSV1 = Path+"\\"+folder1+"\\"+number
                df1 = pd.read_csv(Path_CSV1+"\\計算\\stock_"+number+".csv")
                a1 = (df1.權益總計.astype(int64))
                folder2 = (os.listdir(html_path)[i-2])
                Path_CSV2 = Path+"\\"+folder2+"\\"+number
                df2 = pd.read_csv(Path_CSV2+"\\計算\\stock_"+number+".csv")
                a2 = (df2.權益總計.astype(int64))
                folder3 = (os.listdir(html_path)[i-1])
                Path_CSV3 = Path+"\\"+folder3+"\\"+number
                df3 = pd.read_csv(Path_CSV3+"\\計算\\stock_"+number+".csv")
                a3 = (df3.權益總計.astype(int64))
                a = (df.權益總計.astype(int64))
                b = (df.本期稅前淨利.astype(int64))
                a = (a-a1-a2-a3)
                roe_four = a/(b)
                roe_four = round(roe_four,2)
            else:
                a = (df.權益總計.astype(int64))
                b = (df.本期稅前淨利.astype(int64))
                roe_four = a/(b)
            ...
            df = pd.concat(
                    (pd.read_csv(file,header=0, usecols=['權益總計',
                        '本期稅前淨利',
                        '股票代碼',
                        '股票名稱',
                        '股票類別',
